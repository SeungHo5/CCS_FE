...
const usePlayerStore = create((set, get) => ({
  ...
  setSelectedCharacterId: (id) => set({ selectedCharacterId: Number(id) || null }),
  ...
  loadPlayerData: async () => {
    ...
    if (result.success) {
      console.log('[store] getPlayerData:', result.data);
      const selId =
        Number(
          result.data?.selectedCharacterId ??
          result.data?.selectedCharacter?.id ??
          result.data?.character?.selectedId
      ) || null;

      set({
        currency: result.data.currency,
        level: result.data.level.currentLevel, // 레벨만 저장
        selectedCharacterId: selId,
        isDataLoaded: true,
