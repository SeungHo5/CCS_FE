...
// 미수령 수익 조회 (수령가능 탭) - 상세 정보 포함
export const getPendingRevenue = async () => {
  loadingState.getPendingRevenue = true;
  try {
    const response = await api.get('/auction/sold-items');
    console.log('판매완료된 상품 목록 조회 API 성공:', response.data);
    return { success: true, data: response.data };
  } catch (error) {
    console.log('판매완료된 상품 목록 조회 API 호출 실패:');
...
export const cancelAuction = async (auctionId) => {
  try {
    console.log('판매 취소 API 호출 시작 - auctionId:', auctionId);
    console.log('요청 URL:', `/auction/${auctionId}`);
    console.log('API BaseURL:', api.defaults.baseURL);
    
    // 토큰 상태 확인
    const { accessToken } = useTokenStore.getState();
    console.log('토큰 상태:', accessToken ? '토큰 있음' : '토큰 없음');
    if (accessToken) {
      console.log('토큰 앞부분:', accessToken.substring(0, 20) + '...');
      console.log('토큰 전체 길이:', accessToken.length);
      
      // JWT 페이로드 디코딩해서 userId 확인
      try {
    ...
    console.log('판매 취소 API 성공:', response.data);
    return { success: true, data: response.data };
  } catch (error) {
    console.log('판매 취소 API 호출 실패:');
...
const mapItemData = (items, itemType) => {
  ...
  return items.map(item => {
    ...
    // SoldItemResponseDto의 경우 이미 itemName이 포함되어 있음
    if (item.itemName) {
      mappedItem.name = item.itemName;
      mappedItem.rarity = item.itemRarity;
      // 가격 정보 설정 (salePrice가 판매 가격, amount가 수익 금액)
      mappedItem.price = item.salePrice || item.amount;
      // ID 설정 (수익 수령용)
      mappedItem.id = item.pendingRevenueId || item.id;
      console.log('- 아이템명:', mappedItem.name);
      
      // 이미지 매핑 (SoldItemResponseDto에는 이미지 정보가 없으므로 별도 처리)
      if (itemType === 'character' && item.itemId) {
        const characterData = mergeCharacterData([{ id: item.itemId, amount: 1 }]);
        const character = characterData.find(c => c.id === item.itemId);
        if (character) {
          mappedItem.image = character.image;
        }
      } else if (itemType === 'theme' && item.itemId) {
        const themeData = mergeThemeData([{ id: item.itemId, amount: 1 }]);
        const theme = themeData.find(t => t.id === item.itemId);
        if (theme) {
    ...
    console.log('최종 매핑된 아이템:', mappedItem);
    return mappedItem;
  });
};
  ...
  try {
    ...
    switch (filterType) {
      case 'buy':
        ...
        // 본인이 판매한 상품은 구매 목록에서 제외
        const { user } = useUserStore.getState();
        console.log('현재 사용자 정보:', user);
        console.log('구매 목록 데이터 (필터링 전):', buyData);
        
        if (user && user.id) {
          console.log('사용자 ID:', user.id);
          const filteredData = buyData.filter(item => {
            console.log(`아이템 ${item.itemName || item.name}: sellerId=${item.sellerId}, userId=${user.id}, 필터링=${item.sellerId === user.id ? '제외' : '포함'}`);
            return item.sellerId !== user.id;
          });
          buyData = filteredData;
          console.log('구매 목록 데이터 (필터링 후):', buyData);
        } else {
          console.log('사용자 정보가 없어서 필터링하지 않음');
        }
      ...
      case 'my-sales':
        ...
        const mySalesData = mapItemData(result.data.content || [], itemType);
        return { success: true, data: mySalesData };
...
export const handleAuctionAction = async (filterType, item, activeType, currency = null) => {
  ...
  try {
    ...
    switch (filterType) {
      ...
      case 'my-sales':
        // 판매 취소 - auctionId 사용
        result = await cancelAuction(item.auctionId || item.id);
        break;

      case 'receivable':
        // SoldItemResponseDto에서는 pendingRevenueId를 사용
        result = await receiveRevenue(item.pendingRevenueId || item.id);
        break;
...
export const getPlayerData = async () => {
  ...
  try {
    // 병렬로 코인, 레벨, 캐릭터 정보 조회
    const [currencyResult, levelResult, charactersResult] = await Promise.all([
      getCurrency(),
      getUserLevel(),
      getUserCharacters()
    ]);
    ...
    if (charactersResult.success) {
      const selectedCharacter = charactersResult.data.find(char => char.isSelected);
      selectedCharacterId = selectedCharacter?.characterId || selectedCharacter?.id || null;
    }
