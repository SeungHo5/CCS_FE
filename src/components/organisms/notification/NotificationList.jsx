...
const NotificationList = ({ onCloseSidebar }) => {
  ...
  const handleNotificationPress = async (notification) => {
    ...
    switch (notification.type) {
      case 'FRIEND_REQUEST':
        // 친구 요청 알림 → 사이드바 닫고 친구 관리 페이지로 이동
        console.log('친구 관리 페이지(친구 요청 탭)로 이동');
        if (onCloseSidebar) {
          onCloseSidebar();
        }
        setTimeout(() => {
          navigation.navigate('Friends', { initialTab: 4 }); // filterType 3 = 친구요청
        }, 300);
        break;

      case 'FRIEND_ACCEPT':
      case 'FRIEND_ACCEPTED':
        // 친구 요청 수락 알림 → 그냥 알림 삭제만 (사이드바는 그대로)
      ...
      case 'CHAT_MESSAGE':
        ...
        setTimeout(() => {
          if (notification.targetUserId) {
            navigation.navigate('Chat', { userId: notification.targetUserId });
          }
        }, 300);
        break;
