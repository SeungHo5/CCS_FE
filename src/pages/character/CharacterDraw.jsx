...
const CharacterDraw = () => {
  ...
  const handleDrawPress = async (count) => {
    ...
    if (result.success) {
      ...
      let mappedResults;
      if (type === 'character') {
        // 캐릭터 데이터 매핑 후 뽑기 결과만 필터링
        const allMappedCharacters = mergeCharacterData(results.map(item => ({ 
          id: item.id, 
          amount: 1
        })));
        // 뽑기 결과 ID에 해당하는 것들만 필터링
        mappedResults = results.map(result => {
          const mapped = allMappedCharacters.find(char => char.id === result.id);
          return {
            ...mapped,
            name: result.name || mapped?.name,
            rarity: result.rarity || mapped?.rarity
          };
        });
      } else if (type === 'room') {
        // 테마 데이터 매핑 후 뽑기 결과만 필터링
        const allMappedThemes = mergeThemeData(results.map(item => ({ 
          id: item.id, 
          amount: 1 
        })));
        // 뽑기 결과 ID에 해당하는 것들만 필터링
        mappedResults = results.map(result => {
          const mapped = allMappedThemes.find(theme => theme.id === result.id);
          return {
            ...mapped,
            name: result.name || mapped?.name,
            rarity: result.rarity || mapped?.rarity
      ...
      console.log('[뽑기] 이미지 매핑 완료:', mappedResults);
      ...
      // 결과 반영 (매핑된 결과 사용)
      setRevealedItems(mappedResults);
